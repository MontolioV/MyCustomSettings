...
if (!rootPath) {
  throw new Error(
    'Invalid use of the script: you must provide an absolute path to the target folder as a first argument',
  );
}
    ...
    files.forEach((fileName, fIdx) => {
      ...
      stat(absProjectFolderPath, (err, stats) => {
        ...
        if (stats.isDirectory()) {
          let projectNodeModulesAbsPath = path.join(
            absProjectFolderPath,
            'node_modules',
          );
          try {
            accessSync(
              projectNodeModulesAbsPath,
